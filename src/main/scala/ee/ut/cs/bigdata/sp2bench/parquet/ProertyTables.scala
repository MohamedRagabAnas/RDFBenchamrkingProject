...
object ProertyTables {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val RDFDFJournal = spark.read.format("parquet").load("hdfs://quickstart:8020/user/cloudera/RDFBenchHDFS/Parquet/PropertyTables/Journal").toDF()
    //RDFDFJournal.write.format("org.apache.spark.sql.execution.datasources.orc").save("./Parquet/PropertyTables/Journal")
    RDFDFJournal.createOrReplaceTempView("Journal")
    //spark.sql("create table if not exists Journal as select * from JournalDF")



    val RDFDFinProceedingArticle = spark.read.format("parquet").load("hdfs://quickstart:8020/user/cloudera/RDFBenchHDFS/Parquet/PropertyTables/InProceedingArticle").toDF()
    //RDFDFinProceedingArticle.write.format("org.apache.spark.sql.execution.datasources.orc").save("./Parquet/PropertyTables/InProceedingArticle")
    ...
    val RDFDFPerson = spark.read.format("parquet").load("hdfs://quickstart:8020/user/cloudera/RDFBenchHDFS/Parquet/PropertyTables/Person").toDF()
    //RDFDFPerson.write.format("org.apache.spark.sql.execution.datasources.orc").save("./Parquet/PropertyTables/Person")
    RDFDFPerson.createOrReplaceTempView("Person")
    //spark.sql("create table if not exists Person as select * from PersonDF")

    val RDFDFJournalArticle = spark.read.format("parquet").load("hdfs://quickstart:8020/user/cloudera/RDFBenchHDFS/Parquet/PropertyTables/JournalArticle").toDF()
    //RDFDFJournalArticle.write.format("org.apache.spark.sql.execution.datasources.orc").save("./Parquet/PropertyTables/JournalArticle")
    ...
    val RDFDFDocument = spark.read.format("parquet").load("hdfs://quickstart:8020/user/cloudera/RDFBenchHDFS/Parquet/PropertyTables/Document").toDF()
    //RDFDFJournalArticle.write.format("org.apache.spark.sql.execution.datasources.orc").save("./Parquet/PropertyTables/JournalArticle")
