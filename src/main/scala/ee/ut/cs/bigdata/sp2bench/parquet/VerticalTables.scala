...
object VerticalTables {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //val RDFDFTitle = spark.read.format("parquet").load("file:///data2/VerticalPartionnedTables/VerticalPartitionedTables100/titles").toDF()
    val RDFDFTitle = spark.read.format("parquet").load("hdfs://quickstart:8020/user/cloudera/RDFBenchHDFS/Parquet/VerticalTables/Title").toDF()
    //RDFDFTitle.write.format("org.apache.spark.sql.execution.datasources.orc").save("./Parquet/VerticalTables/title")
    RDFDFTitle.createOrReplaceTempView("Title")
   // spark.sql("create table if not exists Title as select * from TitleRDF")



    //val RDFDFIssued = spark.read.format("parquet").load("file:///data2/VerticalPartionnedTables/VerticalPartitionedTables100/issued").toDF()
    val RDFDFIssued = spark.read.format("parquet").load("hdfs://quickstart:8020/user/cloudera/RDFBenchHDFS/Parquet/VerticalTables/Issued").toDF()
    //RDFDFIssued.write.format("org.apache.spark.sql.execution.datasources.orc").save("./Parquet/VerticalTables/issued")
    ...
    //val RDFDFType = spark.read.format("parquet").load("file:///data2/VerticalPartionnedTables/VerticalPartitionedTables100/type").toDF()
    val RDFDFType = spark.read.format("parquet").load("hdfs://quickstart:8020/user/cloudera/RDFBenchHDFS/Parquet/VerticalTables/Type").toDF()
    //RDFDFType.write.format("org.apache.spark.sql.execution.datasources.orc").save("./Parquet/VerticalTables/type")
    RDFDFType.createOrReplaceTempView("Type")




    //val RDFDFCreator = spark.read.format("parquet").load("file:///data2/VerticalPartionnedTables/VerticalPartitionedTables100/creator").toDF()
    val RDFDFCreator = spark.read.format("parquet").load("hdfs://quickstart:8020/user/cloudera/RDFBenchHDFS/Parquet/VerticalTables/Creator").toDF()
    //RDFDFCreator.write.format("org.apache.spark.sql.execution.datasources.orc").save("./Parquet/VerticalTables/creator")
    RDFDFCreator.createOrReplaceTempView("Creator")
    //spark.sql("create table if not exists Creator as select * from CreatorDF")



    //val RDFDFBookTitle = spark.read.format("parquet").load("file:///data2/VerticalPartionnedTables/VerticalPartitionedTables100/bookTitle").toDF()
    val RDFDFBookTitle = spark.read.format("parquet").load("hdfs://quickstart:8020/user/cloudera/RDFBenchHDFS/Parquet/VerticalTables/BookTitle").toDF()
    //RDFDFBookTitle.write.format("org.apache.spark.sql.execution.datasources.orc").save("./Parquet/VerticalTables/booktitle")
    ...
    //val RDFDFPartOf = spark.read.format("parquet").load("file:///data2/VerticalPartionnedTables/VerticalPartitionedTables100/partOf").toDF()
    val RDFDFPartOf = spark.read.format("parquet").load("hdfs://quickstart:8020/user/cloudera/RDFBenchHDFS/Parquet/VerticalTables/PartOf").toDF()
    //RDFDFPartOf.write.format("org.apache.spark.sql.execution.datasources.orc").save("./Parquet/VerticalTables/partOf")
    RDFDFPartOf.createOrReplaceTempView("PartOf")
    //spark.sql("create table if not exists PartOf as select * from PartOfDF")



    //val RDFDFSeeAlso = spark.read.format("parquet").load("file:///data2/VerticalPartionnedTables/VerticalPartitionedTables100/seeAlso").toDF()
    val RDFDFSeeAlso = spark.read.format("parquet").load("hdfs://quickstart:8020/user/cloudera/RDFBenchHDFS/Parquet/VerticalTables/seeAlso").toDF()
    //RDFDFSeeAlso.write.format("org.apache.spark.sql.execution.datasources.orc").save("./Parquet/VerticalTables/seeAlso")
    RDFDFSeeAlso.createOrReplaceTempView("SeeAlso")




    //val RDFDFPages= spark.read.format("parquet").load("file:///data2/VerticalPartionnedTables/VerticalPartitionedTables100/pages").toDF()
    val RDFDFPages= spark.read.format("parquet").load("hdfs://quickstart:8020/user/cloudera/RDFBenchHDFS/Parquet/VerticalTables/Pages").toDF()
    //RDFDFPages.write.format("org.apache.spark.sql.execution.datasources.orc").save("./Parquet/VerticalTables/pages")
    RDFDFPages.createOrReplaceTempView("Pages")
    //spark.sql("create table if not exists Pages as select * from PagesDF")


    //val RDFDFHomePage= spark.read.format("parquet").load("file:///data2/VerticalPartionnedTables/VerticalPartitionedTables100/homepages").toDF()
    val RDFDFHomePage= spark.read.format("parquet").load("hdfs://quickstart:8020/user/cloudera/RDFBenchHDFS/Parquet/VerticalTables/HomePage").toDF()
    //RDFDFHomePage.write.format("org.apache.spark.sql.execution.datasources.orc").save("./Parquet/VerticalTables/homepage")
    RDFDFHomePage.createOrReplaceTempView("HomePage")
    //spark.sql("create table if not exists HomePage as select * from HomePageDF")


    //val RDFDFAbstract= spark.read.format("parquet").load("file:///data2/VerticalPartionnedTables/VerticalPartitionedTables100/abstract").toDF()
    val RDFDFAbstract= spark.read.format("parquet").load("hdfs://quickstart:8020/user/cloudera/RDFBenchHDFS/Parquet/VerticalTables/Abstract").toDF()
    //RDFDFAbstract.write.format("org.apache.spark.sql.execution.datasources.orc").save("./Parquet/VerticalTables/abstract")
    RDFDFAbstract.createOrReplaceTempView("Abstract")
    //spark.sql("create table if not exists Abstract as select * from AbstractDF")


    //val RDFDFName= spark.read.format("parquet").load("file:///data2/VerticalPartionnedTables/VerticalPartitionedTables100/names").toDF()
    val RDFDFName= spark.read.format("parquet").load("hdfs://quickstart:8020/user/cloudera/RDFBenchHDFS/Parquet/VerticalTables/Name").toDF()
    //RDFDFName.write.format("org.apache.spark.sql.execution.datasources.orc").save("./Parquet/VerticalTables/name")
    RDFDFName.createOrReplaceTempView("Name")
    //spark.sql("create table if not exists Name as select * from NameDF")


    //val RDFDFJournal= spark.read.format("parquet").load("file:///data2/VerticalPartionnedTables/VerticalPartitionedTables100/journal").toDF()
    val RDFDFJournal= spark.read.format("parquet").load("hdfs://quickstart:8020/user/cloudera/RDFBenchHDFS/Parquet/VerticalTables/Journal").toDF()
    //RDFDFJournal.write.format("org.apache.spark.sql.execution.datasources.orc").save("./Parquet/VerticalTables/journal")
    ...
    val RDFDFEditor= spark.read.format("parquet").load("hdfs://quickstart:8020/user/cloudera/RDFBenchHDFS/Parquet/VerticalTables/editor").toDF()
    RDFDFEditor.createOrReplaceTempView("Editor")



    val RDFDFsubclassof= spark.read.format("parquet").load("hdfs://quickstart:8020/user/cloudera/RDFBenchHDFS/Parquet/VerticalTables/subclassof").toDF()
